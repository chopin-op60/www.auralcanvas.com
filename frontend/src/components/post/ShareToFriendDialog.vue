...
const loadFriends = async () => {
  ...
  try {
    ...
    // 修复数据格式问题：后端返回 {friends: [...]}，前端需要提取 friends 数组
    const friendsArray = response.data?.friends || response.data || []
    friends.value = friendsArray
    
    console.log('Loaded friends for sharing:', friendsArray.length, friendsArray)
...
const selectFriend = (friend) => {
  ...
  console.log('Selected friend:', friend?.username, 'ID:', friend?.id)
}
...
const handleShare = async () => {
  ...
  if (!selectedFriend.value.id) {
    console.error('Selected friend missing ID:', selectedFriend.value)
    ElMessage.error('Selected friend data is invalid')
    return
  }
  ...
  try {
    console.log('Sharing post to friend:', selectedFriend.value.username, 'ID:', selectedFriend.value.id)
    
    // 创建或获取对话
    console.log('Creating conversation with user ID:', selectedFriend.value.id)
    ...
    await sharePostToConversation(conversation.id, shareData)
