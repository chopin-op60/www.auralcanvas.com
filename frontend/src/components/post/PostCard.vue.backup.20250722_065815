    ...
    <div class="original-post" :class="{ 'is-repost': post.is_repost }">
      ...
      <div class="post-actions">
        ...
        <el-dropdown trigger="click" @command="handleShareCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="repost">
                <el-icon><Refresh /></el-icon>
                {{ isReposted ? 'Remove Repost' : 'Repost' }}
              </el-dropdown-item>
              <el-dropdown-item command="share-to-friend">
...
import { 
  ...
  ArrowDown, Refresh, ChatDotRound
...
import ShareToFriendDialog from './ShareToFriendDialog.vue'
...
const canDelete = computed(() => {
  ...
  console.log('- Current user ID:', userStore.userId, typeof userStore.userId)
  console.log('- Post user ID:', props.post.user_id, typeof props.post.user_id)
  console.log('- Is repost:', props.post.is_repost)
  console.log('- Repost user ID:', props.post.repost_user?.id, typeof props.post.repost_user?.id)
  
  if (!userStore.isLoggedIn || !userStore.userId) {
    console.log('Delete permission: NO - Not logged in')
    return false
  }
  
  const currentUserId = parseInt(userStore.userId)
  const postAuthorId = parseInt(props.post.user_id || props.post.original_user_id)
  const repostUserId = props.post.repost_user ? parseInt(props.post.repost_user.id) : null
  
  console.log('Normalized IDs:')
  console.log('- Current user:', currentUserId)
  console.log('- Post author:', postAuthorId)
  console.log('- Repost user:', repostUserId)
  
  // 如果是转发，检查当前用户是否为转发者（可以删除转发）
  if (props.post.is_repost && repostUserId) {
    const canDeleteRepost = currentUserId === repostUserId
    console.log('Delete permission for repost:', canDeleteRepost)
    return canDeleteRepost
  }
  
  // 如果是原帖，检查当前用户是否为作者
  const canDeleteOriginal = currentUserId === postAuthorId
  console.log('Delete permission for original post:', canDeleteOriginal)
  return canDeleteOriginal
})
...
const handleCommand = async (command) => {
  console.log('PostCard: Handle command:', command)
  if (command === 'delete') {
    await handleDelete();
  }
}
...
const handleDelete = async () => {
  console.log('PostCard: Starting delete process for post:', props.post.id)
  ...
  try {
    ...
    console.log('PostCard: User confirmed deletion, calling API...')
    
    await deletePost(props.post.id);
    console.log('PostCard: Delete API call successful')
...
// Share dropdown command handler
const handleShareCommand = async (command) => {
  if (!userStore.isLoggedIn) {
    ElMessage.warning('Please login to share')
    return
  }

  if (command === 'repost') {
    await handleRepost()
  } else if (command === 'share-to-friend') {
...
const handleRepost = async () => {
  ...
  if (parseInt(originalAuthorId) === parseInt(userStore.userId)) {
...
onMounted(() => {
  ...
  console.log('PostCard mounted:', {
    ...
    postAuthor: props.post.user_id,
    isRepost: props.post.is_repost
  })
})
</script>
